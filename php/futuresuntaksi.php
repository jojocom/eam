...
<div id="header_wrapper" class="wrapper">
    ...
    <!-- <div id="header"> -->
        ...
        <div id="breadcrump">
            <a href="index_profile.php">Αρχική Σελίδα</a> > <a href="asfalismenos_profile.php">Ασφαλισμένος</a> > <a href="suntaksi.php">Υπολογισμός Ποσού Μελλοντικής Σύνταξης</a>
        </div>
...
<div id="main_wrapper" class="wrapper">
    <form name="calculation" id="calculation" action="futuresuntaksi.php" method="post" align="center">
        <table align="center">
            <tr>
                <th>Έτος</th>
                <th>Αποδοχές</th>
        ...
        <div id="calc" align="right">
            ...
            <?php
                if (isset($_POST['2008'])) {
                    ...
                    $query = "SELECT * FROM `users` WHERE username='$username' AND `type`='Ασφαλισμένος'";
                    ...
                    if($rows == 1) {
                        ...
                        // select to see if username exists
                        $query1 = "SELECT * FROM `asfalismenoi` WHERE username='$username'";
                        $result = $conn->query($query1);
                        if(! $result) die($conn->error);
                        $rows = $result->num_rows;
                        if($rows == 0) {
                            $query2 = "INSERT INTO `asfalismenoi`(`username`, `suntaksi`) VALUES ('$username','$sum')";
                            $result = $conn->query($query2);
                        } else {
                            $query2 = "UPDATE `asfalismenoi` SET `suntaksi`='$sum' WHERE `username`='$username'";
                            $result = $conn->query($query2);
                        }
                    }
                }
